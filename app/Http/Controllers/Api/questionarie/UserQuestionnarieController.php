...
class UserQuestionnarieController extends Controller
{
    ...
    public function UsersquestionnariesByGroup(int $groupId, int $questionariesId)
    {
        $data = UserQuestionnarie::query()
            ...
            ->where('user_questionnarie.group_id', $groupId)
            ->where('user_questionnarie.questionnarie_id', $questionariesId) // Filtrar solo por rol "evaluador"
            ->select('u.id as user_id', 'u.name as user_name', 'user_questionnarie.id as id', 'user_questionnarie.state as state', 'user_questionnarie.deadline as deadline')
            ->distinct()
            ->get();
        return response()->json($data, 200);
    }
